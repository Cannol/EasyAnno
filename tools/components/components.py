...
class DProgressBar(ProgressBar):
    ...
    def click_event(self, event):
        _percentage = event.x / self._width
        if self._percentage_bottom < _percentage:
            return
        super().click_event(event)
        # self.update()
        # self._percentage = event.x / self._width
        # self.coords(self._bar, 0, 0, self._width * self._percentage, self._height)
        # self._hook_click(self._percentage)
...
class PlayController(tkk.Frame, metaclass=LoggerMeta):
    ...
    def __init__(self, master, canvas_workspace, cnf={}, **kw):
        ...
        self.label_state_panel.grid(row=0, column=0, columnspan=4, sticky=tkk.W)
        self.scale.grid(row=0, column=4, columnspan=2, sticky=tkk.E)
        ...
        self.label_interval.grid(row=2, column=4, sticky=tkk.E)
        self.input_interval.grid(row=2, column=5, sticky=tkk.E)
        ...
        self._pre_show = False
        

    def set_data(self, data_frames: list,
                 fps_default: int = 10,
                 interval_default: int = 1, pre_show=False):
        ...
        if self._total_frames == 0:
            ...
            self._bind_workspace.delete('background')
            self.imgTK = None
            self.tk_image = None
            self._bind_workspace['state'] = tkk.DISABLED
            return
        ...
        self.frame_height = data_frames[0].height
        self.frame_width = data_frames[0].width

        # justify scale
        win_w, win_h = self._bind_workspace.winfo_width(), self._bind_workspace.winfo_height()
        rate = min(win_h/self.frame_height, win_w/self.frame_width)
        self.width = int(self.frame_width*rate)
        self.height = int(self.frame_height*rate)
        self.scale.RateVar = rate
        
        self.set_all_panel()
        # self.change_scale(rate)
        ...
        if pre_show:
            ...
            # self.label_frame_number_max['text'] = '%d' % 0
            self._pre_show = True
            self.set_message('预览模式仅显示第一帧内容，点选开始标注后解锁面板')
    ...
    def start_use(self):
        if self._pre_show:
            ...
            self.progress_bar.Click = True
            self._pre_show = False
            self._bind_workspace['state'] = tkk.NORMAL
            self.clear_status()
    ...
    def update_workspace(self):
        img = self.data_frames[self._current_frames - 1]
        if img is None:
            self._L.error("Empty image at frame index: %s" % (self._current_frames-1))
        self.imgTK = ImageTk.PhotoImage(img.resize((self.width, self.height)))
        
        if self.tk_image is None:
            self.tk_image = self._bind_workspace.create_image(0, 0, anchor='nw', image=self.imgTK, tag="background")
        else:
            self._bind_workspace.itemconfig(self.tk_image, image=self.imgTK)
        # print(self.imgTK, self.tk_image, (self.width, self.height))
        self._bind_workspace.frame_change()
    ...
    def change_scale(self, rate):
        ...
        self.height = int(self.frame_height*rate)
        # if self.widt
        self._bind_workspace.config(width=self.width, 
                                    height=self.height)
        # offy = (self._bind_workspace.winfo_height() - self.height)//2
        # offx = (self._bind_workspace.winfo_width() - self.width) //2
        # print(self._bind_workspace.xview())
        # print(self._bind_workspace.yview())
    ...
    # def change_frame_norecall(self, frames: str):
    #     t = int(frames)
    #     if t == self._current_frames:
    #         return
    #     self._current_frames = min(max(1, t), self._total_frames)
    ...
    def Play(self, value: bool):
        if value and self._play_state:
            ...
            # play
            self.buttons[self.BTN_PLAY_PAUSE]['text'] = '暂停\n| |'
            self.panel_locker(tkk.DISABLED)
            ...
            self._play_state = False
            self.buttons[self.BTN_PLAY_PAUSE]['text'] = '播放'
            self.panel_locker(tkk.NORMAL)
    ...
    def set_message(self, msg):  self.label_state_panel['text'] = msg
    ...
    def set_new_position(self, percentage):
        ...
        self.set_all_panel()
        # SharedNamespace.frameseq_panel.set_frame_no_recall(self._current_frames-1)
