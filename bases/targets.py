...
class Target(JsonTransBase, metaclass=LoggerMeta):
    ...
    @classmethod
    def GetAllTargets(cls, path=None):
        path = cls._default_path if path is None else path
        ...
        for n, file in enumerate(targets):
            ...
            try:
                ...
                continue
            # name, _ = os.path.splitext(file.strip())
    ...
    def get_frame_flag(self, frame_index): return self.key_frame_flags[frame_index, 2]
    ...
    def _rand_name_target(self, length=6):
        while True:
            hash_ = hashlib.md5()
    ...
    @classmethod
    def stop_auto(cls, wait_time=None):
        if cls.auto_th is not None:
            cls.auto_save = False
            cls._L.info('Stopping auto-saving thread...')
            cls.auto_th.join(wait_time)
