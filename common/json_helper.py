...
class JsonTransBase(object):
    ...
    def MakeNewFromJsonFile(cls, json_file, ordered_dict=False):
        ...
        o = cls._GetObj(o_dict)
        o.__file = json_file
        return o
    
    def save_default(self, save_file_path=None):
        if save_file_path:
            self.Json = save_file_path
            return
        try:
            self.Json = self.__file
        except AttributeError:
            _L.error('Lack of self.__file attribute to save default file, you have to point out where to save by "save_file_path"')
